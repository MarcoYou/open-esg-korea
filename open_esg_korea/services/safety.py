"""외부(KRX) 실패를 크래시 대신 정상 응답으로 낮추는 안전망 — tool 래퍼 한 곳에서 쓴다.
...
EXTERNAL_ERRORS = (KrxClientError, httpx.HTTPError, asyncio.TimeoutError, TimeoutError)


def classify(exc: BaseException) -> tuple[str, str]:
    if isinstance(exc, (httpx.TimeoutException, asyncio.TimeoutError, TimeoutError)):
        return ("timeout", "KRX ESG 포털 응답이 지연되고 있습니다. 잠시 후 다시 시도하세요.")
    if isinstance(exc, httpx.HTTPStatusError) and exc.response is not None:
        code = exc.response.status_code
        if code in (403, 429):
            return ("blocked", "KRX ESG 포털이 요청을 거부했습니다(차단 또는 과호출). 한 번에 여러 회사를 "
                               "조회 중이라면 건수를 줄이고 간격을 두세요.")
        if 500 <= code < 600:
            return ("upstream_5xx", "KRX ESG 포털이 일시적 오류(5xx)를 반환했습니다. 잠시 후 다시 시도하세요.")
    if isinstance(exc, KrxClientError):
        return ("bad_response", str(exc) or "KRX ESG 포털 응답을 해석할 수 없습니다.")
    return ("transient", "KRX ESG 포털 조회가 일시적으로 실패했습니다. 잠시 후 다시 시도하세요.")
