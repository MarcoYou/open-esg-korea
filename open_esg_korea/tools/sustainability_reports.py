...
def register_tools(mcp):
    ...
    async def sustainability_reports(company: str, from_year: int | None = None, to_year: int | None = None,
                                     format: str = "md") -> str:
        """desc: 지속가능경영보고서 목록 — 연도별 보고서, 작성기준(GRI/SASB/TCFD/UN SDGs), 제3자 검증기관, KIND 접수번호와 원문 링크.
        when: "보고서 냈어?", "GRI 기준 썼나", "검증은 누가", "보고서 원문 링크".
        rule: 목록은 KRX 포털이 거래소 공시(KIND)에서 집계한 것이다. 없다고 나와도 회사가 자사 사이트에만 올렸을 수 있다. 접수번호는 KIND 번호이지 DART 번호가 아니다 — 같은 번호로 DART 를 열면 다른 공시가 나온다. 본문은 링크로 확인한다.
