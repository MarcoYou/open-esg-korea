#!/usr/bin/env python3
"""KIND 원문을 읽는 도구들의 실서버 스모크 — fixture 가 아니라 진짜 KRX·KIND 를 친다.

사용: uv run python scripts/smoke_kind.py [회사명 …]
...
`governance_report`(지배구조 원문)와 `sustainability_reports`(자율공시 서식) 둘 다 본다.
...
async def check_sustainability(company: str) -> int:
    """자율공시 서식(61979) 이 아직 읽히는지. 목록이 있는데 원문 항목이 안 나오면 서식이 바뀐 것이다."""
    try:
        payload = await build_sustainability_reports_payload(company)
    except Exception as exc:                                  # noqa: BLE001
        print(f"  ✗ 지속가능: {type(exc).__name__}: {exc}")
        return 1
    data = payload["data"]
    detail = data.get("detail") or {}
    reports = data.get("reports") or []
    print(f"  {'·' if not reports else '✓'} 지속가능: 목록={len(reports)} 원문={detail.get('year', '-')} "
          f"서식={detail.get('form_no', '-')} 첨부={len(detail.get('attachments') or [])} "
          f"사이트={detail.get('publisher_url', '-')}")
...
async def run(companies: list[str]) -> int:
    ...
    for company in companies:
        ...
        print(f"{'·' if payload['status'] == 'no_data' else '✓'} {company} 지배구조: status={payload['status']} "
        ...
        if data.get("form") == "gov_report" and counts.get("principles") != 28:
            ...
            failed += 1
        failed += await check_sustainability(company)
    return failed
