...
| 4a ✅ | GIR 명세서 ↔ 보고서 공시치 **범위 정렬** — 경계(국내규제/국내전체/글로벌)·Scope 2 방식(지역기반/시장기반)·NF3 포함 여부를 밝혀 나란히 놓는다. Scope 3 는 GIR 에 없어 보고서가 유일한 출처 | 「GIR 과 보고서 숫자가 왜 다르냐」에 **범위 차이**로 답함 · `ghg_emissions(report=True)` · 테스트 159개 |
| 4b ✅ | 기관 **내** 분포 — 「이 등급 이상 몇 사(몇 %)·동점 몇 사」를 세어 준다. **백분위는 만들지 않는다** | 「좋은 편인가」에 기관 간 비교 없이 답함 · `esg_ratings` · 테스트 170개 |
| ~~기관 간 정규화 점수~~ | 폐기 — Purpose(「기관 간 등급을 한 줄에 놓고 비교하지 않는다」)와 평가기관 저작물 가공 금지에 정면으로 어긋난다. 열린 질문 1 을 이것으로 닫는다 | |
...
#### 격자(`table=True`)를 붙여 텍스트와 비교한 결과 (삼성전자 수치 구역 11쪽, 쉼표 있는 값 605개)

| 방식 | 값 보존 | 구조 |
|---|---|---|
| 정렬 텍스트(`page=`) | **605/605 = 100%** | 없음(줄 단위) |
| 격자(`table=True`) | 589/605 = **97.4%** | 있음(행×열) |

- 격자는 왼쪽 표를 정확히 복원한다: `재생에너지 전환율 | % | 93.1 | 23.2 | 93.4 | 24.3 | 93.4 | 24.8`.
- 유실 16개는 **좌우 2단 조판** 쪽에 몰린다. `307,325` 가 `3` + `07,325` 로 갈리고, 「폐기물 처리량」이
  `폐` + `기물 처리량` 으로 쪼개진다.
- **의심 칸 수가 유실을 예측한다**: 의심 0인 6쪽은 유실도 0, 의심 20인 74쪽은 유실 10. 그래서
  격자 칸이 평문(pypdfium2, 숫자 깨짐 0) 토큰에 없으면 `⚠` 를 붙여 돌려준다 — 고칠 순 없어도
  **어디를 믿으면 안 되는지는 말해 준다.**
- 그래서 격자는 **기본값이 아니다.** 값이 필요하면 `table=True` 로 받되 의심 칸 수를 보고,
  0이 아니면 정렬 텍스트와 대조한다.

- 첨부 PDF 크기 분포(실측): 삼성 4.0MB · LG화학 6.0MB · 포스코홀딩스 7.7MB · SK하이닉스 10.4MB(국·영문 2건) ·
  현대차 56.9MB · NAVER 80.8MB. 셀트리온은 첨부 없음.

### 4b 를 정하며 잰 것 — 왜 백분위가 아닌가 (2026-09-08, 2025년 795사)
...
- 등급이 6~7단계뿐이라 **동점이 30~60%**다. 한국ESG연구소 A 등급은 「상위 6%」일 수도 「상위 67%」일 수도 있다
  — 「상위 12%」라고 답하면 지어낸 정밀도다. 그래서 **세어서 그대로** 준다(이 등급 이상 N사 · 동점 M사).
- **모집단이 기관마다 다르다.** MSCI 는 795사 중 74사(대형주 위주)만 평가한다 — 분모를 감추면
  「전체 상장사 중 상위권」으로 오해된다. 커버리지 %를 함께 싣는다.
- 미평가(`-`)는 분모에서 뺀다 — 나쁜 등급이 아니라 평가 대상이 아닌 것이다.
- 효용 확인(삼성전자 2025): 서스틴베스트 **B** → 「이 등급 이상 175사(88.4%)」. 등급만 봐서는 하위권인 줄 모른다.

### 4a 를 정하며 잰 것 — GIR ↔ 보고서 대조 (2026-09-08, 실호출)

2024년 배출량을 **같은 범위로 맞춰** 놓고 보면 두 소스는 사실상 같은 값이다. 회사가 GIR 에 낸 명세서를
보고서에도 싣기 때문이다(현대차 검증의견서 p139 가 「배출권거래제 배출량 보고 및 인증에 관한 지침」을 명시).

| 회사 | GIR 명세서 | 보고서에서 같은 범위 | 차이 |
|---|---|---|---|
| 포스코홀딩스(별도) | 10,745 | **10,746** | 1 tCO₂eq (2022·2023 은 완전 일치) |
| 현대차 | 1,526,375 | **1,526,385** | 10 tCO₂eq |
| SK하이닉스 | 4,848,541 | **4,843,611** | 4,930 (0.10%) |
| LG화학 | 8,351,819 | **8,310,445**(국내) | 41,374 (0.50%) |
| 삼성전자 | 13,594,735 | ≈14,890,000(글로벌) | 130만 (9%) — **범위가 다르다** |

- 현대차: 보고서 헤드라인 2,097,809 = 「국내 전 사업장 + 해외 11개 법인」(p135). 해외 11개 공장 합계
  571,424(Scope 1 219,392 + Scope 2 시장기반 352,032, p141)를 빼면 GIR 과 10톤 차이다.
- **표지에 박히는 숫자는 대개 글로벌**이라 GIR(국내 규제 대상)과 9~37% 벌어진다. 같은 값을 다르게 센 것이 아니다.
- **한 보고서 안에 값이 여럿이다.** SK하이닉스 2024년은 셋: 4,843,611(국내·지역기반·NF3 제외, p87 검증의견서) ·
  5,502,136(글로벌·시장기반, p72) · 5,717,404(국내 + NF3 873,793, p87 합계).

→ 그래서 4a 가 할 일은 「차이를 보여주기」가 아니라 **「범위를 밝혀 주기」**다. 축은 셋:
**경계**(국내규제 / 국내전체 / 글로벌) · **Scope 2 방식**(지역기반 / 시장기반) · **선택 항목**(NF3 포함 여부).
셋이 같으면 GIR 값을 그대로 쓰면 되고, 다르면 그 차이가 곧 답이다.
→ **값을 단정하지 않는다.** 보고서에서 숫자를 뽑아 필드로 만들면 어느 범위인지 잃는다 — 원문 발췌와
축 표시를 주고 판단은 넘긴다(2b-2b 에서 정한 것과 같은 이유).

## 9. 열린 질문

1. ~~등급 스케일을 기관 간 비교 가능한 공통 점수로 변환할지~~ → **원값만 준다**(2026-09-08 확정).
   스케일도 방법론도 다르고, 변환은 평가기관 저작물의 가공이다. 「좋은 편인가」는 **기관 내 백분위**(4b)로 답한다.
