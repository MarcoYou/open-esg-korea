...
| 2b-2a ✅ | 지속가능경영보고서 **자율공시 서식**(KIND 61979) — 보고 대상 기간·목차·검증기관·회사 공개처·첨부 PDF 주소 | 「삼성전자 보고서 무슨 기간 다루나」「PDF 어디서 받나」에 답함 · 테스트 128개 |
| 2b-2b ✅ | 지속가능경영보고서 **PDF 본문** — 공백 무시 검색·쪽 발췌·열 정렬 보기 (`sustainability_report_text`) | 「삼성전자 Scope 3 뭐라고 썼나」에 쪽 번호와 원문으로 답함 · 테스트 145개 |
...
### 2b-2a 에서 확인한 것 (2026-09-07, 로컬 실호출)

- 지속가능경영보고서는 접수번호 하나에 문서가 둘이다: 본문 = 자율공시 서식 `61979`(10~12KB),
  첨부 = `99998` 기타공개첨부서류 안에 **PDF 본체가 상대 링크**로 걸린다(삼성전자 2025: 4.0MB, Range 지원).
  즉 **PDF 는 KIND 에서 키 없이 받아진다** — 회사 사이트를 긁을 필요가 없다.
- 자율공시 서식은 지배구조보고서와 달리 xforms 표 하나다. 왼쪽 칸이 「1. 보고서 명칭」 식 항목명, 오른쪽이 값.
  삼성전자·현대차·SK·SK하이닉스에서 항목 8개가 같았다. 항목은 **번호가 아니라 이름**으로 잇는다.
- 목록에 없고 서식에만 있는 것: **보고 대상 기간**(「2024년 1월 1일부터 12월 31일까지」), 목차, 회사 공개처 URL,
  작성기준 서술(목록은 GRI/SASB/TCFD/SDGs 아이콘 넷뿐이다).
- 원문은 **한 건만** 읽는다(목록 7건을 다 읽으면 21회 호출). KIND 가 막히면 목록은 살리고 경고만 남긴다.

### 2b-2b 에서 잰 것 — 파서 선택 근거 (2026-09-07, 삼성전자 87쪽 4.0MB)

| 파서 | 라이선스 | 전체 시간 | 자간깨짐 | 숫자깨짐 | 표 |
|---|---|---|---|---|---|
| **pypdfium2** | Apache-2.0/BSD-3 | **0.3초** | 898 | 0 | 평문 |
| pypdf | BSD-3 | 7.2초 | 1,854 | **56** (`567 ,056`) | 평문 |
| **pdfplumber `layout=True`** | MIT | 14.2초 | 853 | 0 | **열 정렬 유지** |
| OpenDataLoader | Apache-2.0 | 8.3초 | 994 | 0 | 서술표 ✓ / **수치표 평문 + 빈칸 소실** |

- **훑기 pypdfium2 + 보여주기 pdfplumber** 로 갈랐다. 전 페이지를 pdfplumber 로 뽑으면 48배 느리고,
  pypdf 로 뽑으면 숫자가 깨진다.
- **자간 공백은 파서 문제가 아니다.** `x_tolerance` 를 2~5 로 바꿔도 그대로다 — PDF 안에 진짜 공백이 박혀 있다.
  그래서 검색은 공백을 지우고 한다(「온실가스배출량」: 그대로 0쪽 → 공백무시 16쪽).
- 배제: **PyMuPDF**(AGPL — 이 저장소 라이선스와 충돌), **OpenDataLoader**(Java 런타임 필요, 수치표에서 빈 칸을
  버려 값↔열 대응이 무너진다), camelot/tabula(괘선 필요 — 실측 검출 0개, Java), VLM 계열(torch 수백 MB).
- **HTTP Range 로 부분만 읽기**도 만들어 재봤다: 되지만 이득이 없다. 삼성 4MB 는 35요청 1.0초(통짜 GET 0.4초),
  네이버 81MB 는 250요청 7.9초(통짜 6.9초). pypdf/pdfplumber 가 여는 순간 20~56%를 끌어오고, 왕복 250번은
  간격 0.5초 규칙과도 충돌한다 — 통짜로 받는 쪽이 빠르고 예의 바르다.
- 첨부 PDF 크기 분포(실측): 삼성 4.0MB · LG화학 6.0MB · 포스코홀딩스 7.7MB · SK하이닉스 10.4MB(국·영문 2건) ·
  현대차 56.9MB · NAVER 80.8MB. 셀트리온은 첨부 없음.
